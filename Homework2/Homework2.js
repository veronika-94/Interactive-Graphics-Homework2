...
//Tree Dimensions
var trunkHeight = 20.0;
var trunkWidth = 2.0;
var treehairHeight = 9.0;
var treehairWidth = 9.0;
...
var movement = [null, null, false, null, false, null, false, null, false, null, null];
...
function initNodesBear(Id) {
    ...
    case torsoId:
    	m = translate(x_bear, y_bear, z_bear);
    ...
    case headId:
    	...
    	bear[headId] = createNode( m, head, leftUpperArmId, null);
    	break;
...
function initNodesTree(id) {
    ...
    switch (id) {
        ...
        case trunkId:
            m = translate(initTree_x, initTree_y, initTree_z);
...
function legMovement(id){
	...
	switch(id){
		...
		case leftUpperLegId:
          	...
          	if(theta[id] <= -20){movement[id] = false;}
          	if(theta[id] >= 20){movement[id] = true;}
          	if(movement[id]) theta[id]-= 1; 
          	else theta[id]+= 1; 
        	break;
...
//	x_bear = 6;
	...
	if(theta[torsoId] > -90){movement[torsoId] = true;}
    if(theta[torsoId] <= -90){movement[torsoId] = false;}
    if(movement[torsoId]) theta[torsoId]+= -3;
    else theta[torsoId]+=0;
    
    if(theta[headId] >= 90){movement[headId] = false;}
    if(theta[headId] < 90){movement[headId] = true;}
    if(movement[headId]) theta[headId]+= 3;
    else theta[headId]+=0;
    
	if(theta[rightUpperLegId] > -90){movement[rightUpperLegId] = true;}
    if(theta[rightUpperLegId] <= -90){movement[rightUpperLegId] = false;}
    if(movement[rightUpperLegId]) theta[rightUpperLegId]+= -3;
    else theta[rightUpperLegId]+=0;
    
	if(theta[leftUpperLegId] > -90){movement[leftUpperLegId] = true;}
    if(theta[leftUpperLegId] <= -90){movement[leftUpperLegId] = false;}
    if(movement[leftUpperLegId]) theta[leftUpperLegId]+= -3;
